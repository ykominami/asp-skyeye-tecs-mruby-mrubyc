...
TOPPERS_disint(void)
{
    ...
    Asm("msr CPSR, %0" : : "r"(cpsr) :"memory", "cc");
#else /* THUMB_MODE */
	cpsr = current_sr(); // 現在のステータスレジスタの値
    irq_fiq_mask = cpsr & (0x40|0x80); // 現在のIRQとFIQのマスク
    cpsr |= (0x40|0x80); // IRQとFIQを禁止
	set_sr(cpsr); // ステータスレジスタに設定
#endif /* THUMB_MODE */
...
TOPPERS_set_fiq_irq(uint32_t TOPPERS_irq_fiq_mask)
{
    ...
    Asm("msr CPSR, %0" : : "r"(cpsr):"memory", "cc");
#else /* THUMB_MODE */
	cpsr = current_sr(); // 現在のステータスレジスタ
    cpsr = cpsr & ~(0x40|0x80); // IRQとFIQ以外のビットはそのまま
    cpsr = cpsr | (TOPPERS_irq_fiq_mask & (0x40|0x80)); // IRQとFIQのビットを設定
	set_sr(cpsr); // ステータスレジスタに設定
#endif /* THUMB_MODE */
