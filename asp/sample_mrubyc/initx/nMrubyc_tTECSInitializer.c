 ...
 *   void           cInitialize_initializeBridge( subscript, const mrb_vm* mrb, const struct RClass * TECS );
 ...
 * #[</ENTRY_FUNC>]# */
void
/* eInitialize_initializeBridge(CELLIDX idx, const mrb_vm* mrb) */
eInitialize_initializeBridge(CELLIDX idx, const struct VM * vm)
{
	CELLCB	*p_cellcb;
	int      i;
	struct RClass *TECS;
	...
	/* TECS = mrb_define_module( (mrb_vm *)mrb, "TECS" ); */
	// mrb: const mrb_vm * => mrb_vm *
	TECS = NULL;
    VAR_TECS = TECS;

	for( i = 0; i < NCP_cInitialize; i++ )
		cInitialize_initializeBridge( i, (struct VM *)vm, TECS );
}
