 ...
 *  シリアルポートへの文字送信
 *
 *  p_cellcbで指定されるシリアルI/Oポートに対して，文字cを送信する．文
 *  字を送信レジスタにいれた場合にはtrueを返す．そうでない場合には，送
 *  信レジスタが空いたことを通知するコールバック関数を許可し，falseを返
 *  す．この関数は，CPUロック状態で呼び出される．
 */
Inline bool_t
serialPort_snd_chr(CELLCB *p_cellcb, char c)
{
 ...
 */
static ER_BOOL
serialPort_wri_chr(CELLCB *p_cellcb, char c)
{
	bool_t	buffer_full;
	ER		ercd, rercd;
 ...
 */
static bool_t
serialPort_rea_chr(CELLCB *p_cellcb, char *p_c)
{
	bool_t	buffer_empty;
	ER		ercd;
 ...
 */
bool_t
enSerialPort_getChar(CELLIDX idx, char *p_c)
{
	CELLCB	*p_cellcb;
